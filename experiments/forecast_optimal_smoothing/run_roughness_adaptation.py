...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Isolate latent-trend-roughness regime changes while measuring "
...
def _roughness_transitions(low: float, high: float) -> dict[str, tuple[float, float]]:
    return {
        "stable_low_roughness": (low, low),
        "low_to_high_roughness": (low, high),
        "high_to_low_roughness": (high, low),
        "stable_high_roughness": (high, high),
    }
        ...
        rows.append(
            {
                ...
                "observation_noise_std": float(args.observation_noise_std),
                "horizon": int(horizon),
                "regime_point": rp,
                "outer_origin": origin,
...
def _target_tracking(
    ...
    mappings = (
        (
            "low_to_high_roughness",
            "stable_low_roughness",
            "stable_high_roughness",
        ),
        (
            "high_to_low_roughness",
            "stable_high_roughness",
            "stable_low_roughness",
        ),
    )
...
def main() -> None:
    ...
    if args.low_slope_noise_std <= 0.0 or args.high_slope_noise_std <= 0.0:
        raise ValueError("Slope-noise standard deviations must be positive.")
    ...
    for i, (seed, transition_item, horizon) in enumerate(configs, start=1):
        transition, (pre_slope_noise_std, post_slope_noise_std) = transition_item
        config_start = time.perf_counter()
        print(
            f"[{i}/{len(configs)}] seed={seed} transition={transition} "
            f"slope_sd={pre_slope_noise_std}->{post_slope_noise_std} "
            f"h={horizon}",
            flush=True,
        )
    ...
    run_dir = (
        ...
        / f"{stamp}_roughness-adaptation_{args.preset}_{git_short_sha()}"
    )
    run_dir.mkdir(parents=True, exist_ok=True)

    frame.to_csv(run_dir / "roughness_adaptation_grid.csv", index=False)

    value_summary = _aggregate_summary(frame)
    value_summary.to_csv(
        run_dir / "roughness_adaptive_value_summary.csv",
        index=False,
    )

    paired_value, paired_value_summary = _paired_adaptation_value(frame)
    paired_value.to_csv(
        run_dir / "roughness_paired_adaptation_value.csv",
        index=False,
    )
    paired_value_summary.to_csv(
        run_dir / "roughness_paired_adaptation_value_summary.csv",
        index=False,
    )

    seed_direct, seed_excess = _seed_level_summary(frame, paired_value)
    seed_direct.to_csv(
        run_dir / "roughness_adaptive_value_seed_summary.csv",
        index=False,
    )
    seed_excess.to_csv(
        run_dir / "roughness_paired_adaptation_value_seed_summary.csv",
        index=False,
    )

    tracking, tracking_path, tracking_summary = _target_tracking(frame)
    tracking.to_csv(
        run_dir / "roughness_target_tracking.csv",
        index=False,
    )
    tracking_path.to_csv(
        run_dir / "roughness_target_tracking_path.csv",
        index=False,
    )
    tracking_summary.to_csv(
        run_dir / "roughness_target_tracking_summary.csv",
        index=False,
    )
    ...
    metadata = {
        ...
        "experiment": "latent_trend_roughness_adaptation_value",
        "preset": args.preset,
        "n_seeds": n_seeds,
        "n_obs": args.n_obs,
        "regime_point": args.regime_point,
        ...
        "transitions": {
            name: {
                "pre_slope_noise_std": pre,
                "post_slope_noise_std": post,
            }
